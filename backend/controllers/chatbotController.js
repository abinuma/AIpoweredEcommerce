...
const startSession = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to start chat session" });
  }
};
...
const sendMessage = async (req, res) => {
  try {
    ...
    const { rows: userMessageRows } = await pool.query(
    `INSERT INTO chat_messages (session_id, role, message, created_at)
     VALUES ($1, 'user', $2, $3)
     RETURNING id`,
    [sessionId, message, Date.now()]
...
const assistantMessageId = assistantRows[0].id;
    ...
    await pool.query("UPDATE chat_sessions SET updated_at = $1 WHERE id = $2", [
      Date.now(),
      sessionId,
    ]);
    ...
    if (aiResult.suggestedProducts && aiResult.suggestedProducts.length > 0) {
      const placeholders = aiResult.suggestedProducts
        .map((_, i) => `$${i + 1}`)
        .join(", ");
      const { rows } = await pool.query(
...
export { startSession, sendMessage, getSessionHistory, deleteSession, deleteMessage };
